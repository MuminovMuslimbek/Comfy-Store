        ...
        <div>
          <div className='flex justify-between items-center pb-24'>
            <div className='flex flex-col items-start max-w-[600px]'>
              <h1 className='max-w-[500px] font-bold text-[#F8F8F2] text-4xl sm:text-6xl tracking-tight'>We are changing the way people shop</h1>
              ...
              <button onClick={handleProductPage} className='bg-[#FF7AC6] mt-[40px] px-4 py-3 rounded-lg font-medium text-[#272935] uppercase transition-[0.3s] active:scale-95'>
                our products
              </button>
            </div>
          ...
          <div className='gap-4 grid md:grid-cols-2 lg:grid-cols-3 pt-12'>
            {
              data && data.map((product, index) => (
                <div onClick={() => { handleRedirect(product.id) }} key={index} className='shadow-xl hover:shadow-2xl p-4 rounded-2xl w-full text-[#F8F8F2] transition duration-300 cursor-pointer select-none'>
                  <img className='rounded-xl w-full h-64 md:h-48 object-cover' src={product.attributes.image} alt={product.attributes.title} />
